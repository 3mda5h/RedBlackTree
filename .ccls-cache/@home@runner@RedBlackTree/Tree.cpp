...
{
if(node != NULL && node->parent != NULL && node->parent->sibling != NULL && strcmp(node->parent->color, "red") == 0 && strcmp(node->parent->sibling->color, "red") == 0)
  {
    ...
    if(grandpa == root) cout << "grandpa (" << grandpa->number << ") is root" << endl; 
    cout << "-----------" << endl;
    display();
    caseThree(grandpa);
  }
